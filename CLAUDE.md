...
- **The rail** is a `.card::before` inset rounded marker on the left edge: `left: 0; top/bottom: 16px; width: 4px; border-radius: 0 4px 4px 0; background: var(--card-accent)`. On hover it grows toward the edges (`top/bottom: 11px`). It falls back to `--border-strong` for categories with no accent mapping. This is the one consistent color moment shared by all three card types.
